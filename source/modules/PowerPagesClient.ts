...
import { LookupData } from "./power-pages/LookupData";
...
export default class PowerPagesClient {
  ...
  ): (searchTerm: string) => Promise<Data[]> {
    return async (searchTerm: string) => {
      const results = await Promise.all(
        entities.map(async (entity) => {
          const data = await PowerPagesClient.fetchEntityData(
            entity,
            searchTerm
          );
  ...
  public static async GetFieldTargets(
    ...
    const viewLayoutReferences =
      PowerPagesClient.parseViewLayouts(viewLayoutsData);
    ...
    const viewLayouts = await PowerPagesClient.fetchFullViewLayouts(
      viewLayoutReferences,
      lookupModal
    );
  ...
  private static async fetchFullViewLayouts(
    viewLayoutReferences: pp.ViewLayout[],
    lookupModal: JQuery<HTMLElement>
  ): Promise<pp.CompleteViewLayout[]> {
    const viewLayouts: pp.CompleteViewLayout[] = [];

    // Handle the embedded layout (from DOM) once, if present
    const embedded = viewLayoutReferences[0];
    if (embedded) {
      const firstLayout = plainToInstance(pp.CompleteViewLayout, embedded, {
        exposeDefaultValues: false,
        enableImplicitConversion: true,
        excludeExtraneousValues: false,
      });
      viewLayouts.push(firstLayout);
    }

    // Process the rest via the service
    const lookupDataUrl: string = lookupModal
      .find(".entity-lookup")
      .data("url");
    ...
    for (const layoutRef of viewLayoutReferences) {
      ...
      const response = await PowerPagesClient.apiRequest<LookupData>({
        type: "POST",
        url: lookupDataUrl,
      ...
      const lookupData = plainToInstance(LookupData, response, {
        exposeDefaultValues: false,
        enableImplicitConversion: true,
        excludeExtraneousValues: false,
      });

      if (lookupData?.CompleteViewLayout?.Configuration) {
        viewLayouts.push(lookupData.CompleteViewLayout);
      }
    }
  ...
  private static getEntityFormViewEntityName(): string | null {
    ...
    return entityNameDiv
      ? entityNameDiv.attributes.getNamedItem("value")?.value ?? null
      : null;
  }
